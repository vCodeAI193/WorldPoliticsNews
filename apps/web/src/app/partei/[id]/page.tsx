...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(`${BASE}/api/parties/${id}/analysis?name=${encodeURIComponent(name || id)}`);
    if (res.ok) {
      const data = await res.json();
  ...
  return createMetadata(`${name} - WorldPoliticsNews`, 'Medienanalyse zu dieser Partei', { url: `/partei/${id}` });
}
